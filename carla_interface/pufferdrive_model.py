...
class PufferModule:
    def __init__(self, config, env, model_path, deterministic=False, device=None):
        ...
        self.device = device or torch.device("cuda" if torch.cuda.is_available() and not deterministic else "cpu")
        ...
        if self.recurrent:
            self.lstm_state = {
                "lstm_h": torch.zeros(env.num_agents, policy.hidden_size, device=self.device),
                "lstm_c": torch.zeros(env.num_agents, policy.hidden_size, device=self.device),
            }
    ...
    def step(self, obs):
        ...
        with torch.no_grad():
            ...
            logits, _ = self.policy.forward_eval(obs, self.lstm_state if self.recurrent else None)
