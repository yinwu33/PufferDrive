...
# WOSAC eval
from pufferlib.atlantic.benchmark import metrics
from pufferlib.atlantic.benchmark import estimators
...
class Evaluator:
    ...
    def _unpack_eval_configs(self, configs):
        ...
        eval_config["env"]["fixed_self_fault_factor"] = eval_config["eval"]["fixed_self_fault_factor"]
        eval_config["env"]["fixed_non_self_fault_factor"] = eval_config["eval"]["fixed_non_self_fault_factor"]
    ...
    def log_stats(self):
        ...
        if self.human_replay_stats is not None:
            ...
            for key in (
                "self_fault_collision_rate",
                "non_self_fault_collision_rate",
                "ambiguous_collision_rate",
            ):
                if key in self.human_replay_stats:
                    eval_stats[f"eval/hr_{key}"] = self.human_replay_stats[key]
        if self.self_play_stats is not None:
            ...
            for key in (
                ...
                if key in self.self_play_stats:
                    eval_stats[f"eval/sp_{key}"] = self.self_play_stats[key]
        else:
            return
