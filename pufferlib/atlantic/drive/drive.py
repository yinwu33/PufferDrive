...
class Drive(pufferlib.PufferEnv):
    def __init__(
        ...
        height=1024,
        human_agent_idx=0,
        self_fault_factor_min=0.0,
        self_fault_factor_max=1.0,
        non_self_fault_factor_min=0.0,
        non_self_fault_factor_max=1.0,
        offroad_factor_min=0.0,
        offroad_factor_max=1.0,
        condition_sample_mode="random",
        fixed_self_fault_factor=1.0,
        fixed_non_self_fault_factor=1.0,
        fixed_offroad_factor=1.0,
        collision_reward_mode="adversarial",
        fault_speed_threshold=0.5,
        front_contact_cos_threshold=0.70710678,
        non_self_fault_reward_once=True,
        lane_width_min=1.0,
        lane_width_max=5.0,
        ...
        self.self_fault_factor_min = self_fault_factor_min
        self.self_fault_factor_max = self_fault_factor_max
        self.non_self_fault_factor_min = non_self_fault_factor_min
        self.non_self_fault_factor_max = non_self_fault_factor_max
        ...
        self.fixed_self_fault_factor = fixed_self_fault_factor
        self.fixed_non_self_fault_factor = fixed_non_self_fault_factor
        self.fixed_offroad_factor = fixed_offroad_factor
        self.collision_reward_mode_str = collision_reward_mode
        self.fault_speed_threshold = fault_speed_threshold
        self.front_contact_cos_threshold = front_contact_cos_threshold
        self.non_self_fault_reward_once = int(non_self_fault_reward_once)
        ...
        if self.collision_reward_mode_str == "legacy":
            self.collision_reward_mode = 0
        elif self.collision_reward_mode_str == "adversarial":
            self.collision_reward_mode = 1
        else:
            raise ValueError(
                "collision_reward_mode must be one of 'legacy' or 'adversarial'. "
                f"Got: {self.collision_reward_mode_str}"
            )
        if self.fault_speed_threshold < 0:
            raise ValueError("fault_speed_threshold must be non-negative")
        if not 0.0 < self.front_contact_cos_threshold < 1.0:
            raise ValueError("front_contact_cos_threshold must be between 0 and 1")
        factor_values = {
            "self_fault_factor_min": self_fault_factor_min,
            "self_fault_factor_max": self_fault_factor_max,
            "non_self_fault_factor_min": non_self_fault_factor_min,
            "non_self_fault_factor_max": non_self_fault_factor_max,
            "offroad_factor_min": offroad_factor_min,
            "offroad_factor_max": offroad_factor_max,
            "fixed_self_fault_factor": fixed_self_fault_factor,
            "fixed_non_self_fault_factor": fixed_non_self_fault_factor,
            "fixed_offroad_factor": fixed_offroad_factor,
        }
        if any(value < 0 for value in factor_values.values()):
            raise ValueError(f"Reward factors must be non-negative. Got: {factor_values}")
        if self_fault_factor_max < self_fault_factor_min:
            raise ValueError("self_fault_factor_max must be >= self_fault_factor_min")
        if non_self_fault_factor_max < non_self_fault_factor_min:
            raise ValueError("non_self_fault_factor_max must be >= non_self_fault_factor_min")
        if offroad_factor_max < offroad_factor_min:
            raise ValueError("offroad_factor_max must be >= offroad_factor_min")
        ...
        for i in range(num_envs):
            ...
            env_id = binding.env_init(
                ...
                fixed_non_self_fault_factor=fixed_non_self_fault_factor,
                fixed_offroad_factor=fixed_offroad_factor,
                collision_reward_mode=self.collision_reward_mode,
                fault_speed_threshold=self.fault_speed_threshold,
                front_contact_cos_threshold=self.front_contact_cos_threshold,
                non_self_fault_reward_once=self.non_self_fault_reward_once,
                ...
                map_id=map_ids[i],
                max_agents=nxt - cur,
                ini_file="pufferlib/config/atlantic/adv_drive.ini",
    ...
    def resample_maps(self):
        ...
        for i in range(num_envs):
            ...
            env_id = binding.env_init(
                ...
                self_fault_factor_min=self.self_fault_factor_min,
                self_fault_factor_max=self.self_fault_factor_max,
                non_self_fault_factor_min=self.non_self_fault_factor_min,
                non_self_fault_factor_max=self.non_self_fault_factor_max,
                offroad_factor_min=self.offroad_factor_min,
                offroad_factor_max=self.offroad_factor_max,
                condition_sample_mode=self.condition_sample_mode,
                fixed_self_fault_factor=self.fixed_self_fault_factor,
                fixed_non_self_fault_factor=self.fixed_non_self_fault_factor,
