...
#define MATCH(s, n) strcmp(section, s) == 0 && strcmp(name, n) == 0
    ...
    } else if (MATCH("env", "self_fault_factor_min")) {
        env_config->self_fault_factor_min = atof(value);
    } else if (MATCH("env", "self_fault_factor_max")) {
        env_config->self_fault_factor_max = atof(value);
    } else if (MATCH("env", "non_self_fault_factor_min")) {
        env_config->non_self_fault_factor_min = atof(value);
    } else if (MATCH("env", "non_self_fault_factor_max")) {
        env_config->non_self_fault_factor_max = atof(value);
    ...
    } else if (MATCH("env", "fixed_self_fault_factor")) {
        env_config->fixed_self_fault_factor = atof(value);
    } else if (MATCH("env", "fixed_non_self_fault_factor")) {
        env_config->fixed_non_self_fault_factor = atof(value);
    } else if (MATCH("env", "fixed_offroad_factor")) {
        env_config->fixed_offroad_factor = atof(value);
    } else if (MATCH("env", "collision_reward_mode")) {
        if (strcmp(value, "\"legacy\"") == 0 || strcmp(value, "legacy") == 0) {
            env_config->collision_reward_mode = 0;
        } else if (strcmp(value, "\"adversarial\"") == 0 || strcmp(value, "adversarial") == 0) {
            env_config->collision_reward_mode = 1;
        } else {
            printf("Warning: Unknown collision_reward_mode value '%s', defaulting to adversarial\n", value);
            env_config->collision_reward_mode = 1;
        }
    } else if (MATCH("env", "fault_speed_threshold")) {
        env_config->fault_speed_threshold = atof(value);
    } else if (MATCH("env", "front_contact_cos_threshold")) {
        env_config->front_contact_cos_threshold = atof(value);
    } else if (MATCH("env", "non_self_fault_reward_once")) {
        env_config->non_self_fault_reward_once =
