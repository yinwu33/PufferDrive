...
def _respawn_mask(traj, a, end=None):
    """Per-step bool mask (True once agent ``a`` has respawned) or None if unavailable.

    Once an agent reaches its goal it teleports back to its t=0 spawn pose; drawing it
    there produces a fake overlap with whoever now occupies that spot, so callers blank
    out those steps. Sliced to ``[:end]`` to match the trajectory slice."""
...
def render_rollout(traj, lanes, *, agent_states=None, agent_types=None,
    ...
    for a in range(n_agents):
        is_ego = a == 0
        color = _agent_color(is_ego, agent_types[a] if agent_types is not None else 0)
        xa, ya, ha = x[:end, a], y[:end, a], hd[:end, a]
        if len(xa) == 0:
            continue
        # Blank out post-respawn steps so we don't draw the agent on top of others.
...
def render_rollout_frames(traj, lanes, *, agent_states=None, agent_types=None,
                          reward=None, ego_collision=False, ego_offroad=False, init_invalid=False,
    ...
    for t in frame_ts:
        ...
        for a in range(n_agents):
            ...
            if mask is not None and bool(mask[-1]):
                continue  # agent has respawned by now: drop it instead of faking an overlap
